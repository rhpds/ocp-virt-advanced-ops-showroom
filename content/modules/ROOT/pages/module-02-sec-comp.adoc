...
+
. You will see a node enter the _SchedulingDisabled_ state to begin it's remediation. Click on that node for it's Node details page, and notice that the event log shows the node was cordoned to apply the update, and you can also see CPU and Memory utilization drop off as the node was successfully drained.
+
